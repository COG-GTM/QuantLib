...
QuantLibGlobalFixture::QuantLibGlobalFixture() {
    ...
    // called just to output a message in case a speed flag is passed
    speed_level(argc, argv);
...
SpeedLevel speed_level(int argc, char** argv) {
    ...
    for (int i=1; i<argc; ++i) {
        std::string arg = argv[i];
        if (arg == "--slow" || arg == "--fast" || arg == "--faster")
            BOOST_TEST_MESSAGE("WARNING: test selection via speed (i.e., " << arg << ") is no longer supported.");
    }
    return Slow;
}
